...
function FallbackMap({ accommodations, activeId, onMarkerClick }: Props) {
  return (
    <div className="w-full h-full bg-muted/50 flex flex-col items-center justify-center gap-3 relative overflow-hidden">
      <div className="absolute inset-0 opacity-5">
        <svg viewBox="0 0 100 100" className="w-full h-full">
          <rect fill="hsl(var(--muted))" width="100" height="100" />
        </svg>
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-2 max-w-[200px] mt-1">
        {accommodations.slice(0, 4).map((acc) => (
          <button
            key={acc.id}
            onClick={() => onMarkerClick?.(acc.id)}
...
export function PropertiesMap({ accommodations, activeId, onMarkerClick, className = "h-full w-full" }: Props) {
  ...
  const prevActiveId = useRef<number | null | undefined>(null);

  useEffect(() => {
    if (!mapRef.current || !activeId || activeId === prevActiveId.current) return;
  ...
  if (mapError) {
    return <FallbackMap accommodations={accommodations} activeId={activeId} onMarkerClick={onMarkerClick} />;
  }
  ...
  return (
    <div className={`relative z-0 overflow-hidden ${className}`}>
      <Map
        ...
        initialViewState={{
          longitude: RWANDA_CENTER.longitude,
          latitude: RWANDA_CENTER.latitude,
          zoom: 7.5,
        }}
      ...
      >
        ...
        {accommodations.map((acc) => {
          ...
          return (
            ...
            >
              <div className={`flex flex-col items-center cursor-pointer transition-all duration-200 ${isActive ? 'scale-110 z-50' : 'hover:scale-105 hover:z-40'}`}>
                <div
                  className={`px-2.5 py-1 rounded-full text-xs font-bold shadow-lg border transition-all
                    ${isActive
